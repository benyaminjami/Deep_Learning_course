...
class AutoEncoder(object):
    ...
    def encoder(self, inputs):
        ...
        # convolutional layer
        relu = tf.nn.relu
        conv1 = ConvLayer(input_filters=FLAGS.num_channel, output_filters=8, act=relu, 
                          kernel_size=3, kernel_stride=1, kernel_padding='SAME')(inputs)
        print(conv1.shape)
        # convolutional and pooling layer
        conv_pool1 = ConvPoolLayer(input_filters=8, output_filters=8, act=relu, 
                                   kernel_size=3, kernel_stride=1, kernel_padding='SAME', 
                                   pool_size=3, pool_stride=2, pool_padding='SAME')(conv1)
        print(conv_pool1.shape)
        # convolutional layer
        conv2 = ConvLayer(input_filters=8, output_filters=16, act=relu, 
                          kernel_size=3, kernel_stride=1, kernel_padding='SAME')(conv_pool1)
        print(conv2.shape)
        # convolutional and pooling layer
        conv_pool2 = ConvPoolLayer(input_filters=16, output_filters=16, act=relu, 
                                   kernel_size=3, kernel_stride=1, kernel_padding='SAME', 
                                   pool_size=3, pool_stride=2, pool_padding='SAME')(conv2)
        print(conv_pool2.shape)

        conv3 = ConvLayer(input_filters=16, output_filters=32, act=relu, 
                          kernel_size=3, kernel_stride=1, kernel_padding='SAME')(conv_pool2)
        print(conv3.shape)

        conv_pool3 = ConvPoolLayer(input_filters=32, output_filters=32, act=relu, 
                                   kernel_size=3, kernel_stride=1, kernel_padding='SAME', 
                                   pool_size=3, pool_stride=2, pool_padding='SAME')(conv3)
        ...
        flatten = tf.reshape(conv_pool3, [tf.shape(conv_pool3)[0], flatten_dim])
        print(flatten.shape)
        
        
        # apply fully connected layer

        dense = tf.matmul(flatten, normal_initializer([flatten_dim, FLAGS.code_size])) + zero_initializer([FLAGS.code_size])
        print(dense.shape)
    ...
    def decoder(self, inputs, last_conv_dims):
        ...
        decode_layer = tf.matmul(inputs, normal_initializer([FLAGS.code_size, np.prod(last_conv_dims)])) + zero_initializer([np.prod(last_conv_dims)])
        ...
        deconv_input = tf.reshape(decode_layer, [tf.shape(inputs)[0], last_conv_dims[0], last_conv_dims[1], last_conv_dims[2]])
        ...
        deconv1 = DeconvLayer(input_filters=last_conv_dims[2] , output_filters=16, act=relu, 
                              kernel_size=3, kernel_stride=2, kernel_padding='SAME')(deconv_input)
        print(deconv1.shape)
        # transpose convolutional layer
        deconv2 = DeconvLayer(input_filters=16 , output_filters=8, act=relu, 
                              kernel_size=3, kernel_stride=2, kernel_padding='SAME')(deconv1)
        print(deconv2.shape)
        # transpose convolutional layer
        deconv3 = DeconvLayer(input_filters=8 , output_filters=1, act=tf.keras.activations.linear, 
                              kernel_size=3, kernel_stride=2, kernel_padding='SAME')(deconv2)
        print(deconv3.shape)
    ...
    def _loss(self):
        ...
        labels = tf.reshape(self.input_original_images, [-1, 32*32])
        
        logits = tf.reshape(self.reconstruct, [-1, 32*32])
